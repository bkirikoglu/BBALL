...
nba <- read.csv("2008-09nba.trunc.csv", sep=",")
...
gsw <- subset(shots.known, player == "LeBron James")
...
getColorByPoints <- function(val) {
  ...
  pal <- colorRampPalette(c("#c2fffc", "red"))
  cols <- pal(numCols)
...
d<- c(rep(1, nrow(gsw.agg))) #sqrt(gsw.agg$freq)/3
gridColors <- sapply(ptsper, getColorByPoints)
plot(0, 0, xlim=c(0,50), ylim=c(0,40), type="n", xaxt="n", yaxt="n", xlab="", ylab="", bty="n")
symbols(gsw.agg$x, gsw.agg$y, squares=d, asp=1, inches=FALSE, add=TRUE, bg=gridColors, fg=NA)
